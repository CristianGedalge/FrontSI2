...
<aside class="w-64 bg-slate-900 h-screen fixed left-0 top-0 text-white flex flex-col shadow-2xl z-50">
  ...
  <!-- MENU ITEMS -->
  <nav class="flex-grow p-6 space-y-3 mt-4 overflow-y-auto hide-scrollbar">
    <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4 mb-4">Menú Principal</p>
    
    <a *ngIf="auth.currentUser()?.rol === 'admin'" routerLink="/admin/dashboard" routerLinkActive="bg-primary-500 text-white shadow-lg shadow-primary-500/20" [routerLinkActiveOptions]="{exact: true}" 
    ...
    <a *ngIf="auth.currentUser()?.rol === 'admin'" routerLink="/admin/mi-taller" routerLinkActive="bg-primary-500 text-white shadow-lg shadow-primary-500/20"
       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 group-hover:scale-110 transition-transform">
        <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349m-16.5 11.65V9.35m0 0a3.001 3.001 0 003.75-.615A2.993 2.993 0 009.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 002.25 1.016c.896 0 1.7-.393 2.25-1.016a3.001 3.001 0 003.75.614m-16.5 0a3.004 3.004 0 01-.621-4.72L4.318 3.44A1.5 1.5 0 015.378 3h13.243a1.5 1.5 0 011.06.44l1.19 1.189a3 3 0 01-.621 4.72m-13.5 8.65h3.75a.75.75 0 00.75-.75V13.5a.75.75 0 00-.75-.75H6.75a.75.75 0 00-.75.75v3.75c0 .415.336.75.75.75z" />
      </svg>
      <span class="font-bold text-sm">Mi Taller</span>
    </a>
    ...
    <a *ngIf="auth.currentUser()?.rol === 'admin'" routerLink="/admin/servicios-en-vivo" routerLinkActive="bg-primary-500 text-white shadow-lg shadow-primary-500/20"
       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 group-hover:scale-110 transition-transform">
        <path stroke-linecap="round" stroke-linejoin="round" d="M15.042 21.672 13.684 16.6m0 0-2.51 2.225.569-9.47 5.227 7.917-3.286-.672ZM12 2.25V4.5m5.834.166-1.591 1.591M20.25 10.5H18M7.757 14.743l-1.59 1.59M6 10.5H3.75m4.007-4.243-1.59-1.59" />
      </svg>
      <span class="font-bold text-sm">En Vivo</span>
    </a>

    <a *ngIf="auth.currentUser()?.rol === 'superadmin'" routerLink="/super-admin/dashboard" routerLinkActive="bg-primary-500 text-white shadow-lg shadow-primary-500/20"
       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 group-hover:scale-110 transition-transform">
        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6v7.5m16.5-7.5v7.5m-16.5 4.5h16.5M4.5 9h15M5.25 12h13.5m-14.25 3h15m-15.75 3h16.5" />
      </svg>
    ...
    <a *ngIf="auth.currentUser()?.rol === 'superadmin'" routerLink="/super-admin/reportes" routerLinkActive="bg-primary-500 text-white shadow-lg shadow-primary-500/20"
       class="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 group-hover:scale-110 transition-transform">
        <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" />
      </svg>
      <span class="font-bold text-sm">Reportes IA</span>
    </a>
  </nav>
