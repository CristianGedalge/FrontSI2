    ...
    <div *ngFor="let taller of talleres()" class="bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_10px_40px_rgba(0,0,0,0.02)] hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden">
      ...
      <div class="p-8 -mt-16 relative">
        ...
        <div class="space-y-3 mb-8">
          ...
          <div class="flex items-center justify-between text-xs py-2 border-b border-dashed border-slate-100">
            <span class="text-slate-400 font-bold uppercase tracking-tighter">Administrador:</span>
            <span class="text-slate-700 font-medium">{{ taller.admin_nombre || 'Desconocido' }}</span>
          </div>
        </div>

        <div class="flex gap-2">
          <a [href]="'https://www.google.com/maps/search/?api=1&query=' + taller.latitud + ',' + taller.longitud" target="_blank"
             class="flex-1 flex items-center justify-center gap-2 py-3 text-xs font-black uppercase tracking-widest text-blue-500 bg-blue-50 rounded-xl hover:bg-blue-500 hover:text-white transition-colors">
